            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">路线收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="luxianName" id="luxianName" placeholder="路线名" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="luxianTypes" id="luxianTypes" lay-filter="luxianTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in luxianTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../luxian/detail.jsp?id='+item.luxianId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.luxianPhoto?item.luxianPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../luxian/detail.jsp?id='+item.luxianId)"><span>{{item.luxianName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.luxianNewMoney"><span style="color: red;">{{item.luxianNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteLuxianCollection(item.id)">删除 {{item.luxianName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 luxianCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            luxianCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteLuxianCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('luxianCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#luxianName').val() != null && jquery('#luxianName').val() != "" && jquery('#luxianName').val() != "null" ) {
                param['luxianName'] = jquery('#luxianName').val();
            }
                                                             
            var luxianTypes = $("#luxianTypes").find("option:selected").val();
            if (luxianTypes != null && luxianTypes != "" && luxianTypes != "null") {
                            param['luxianTypes'] = luxianTypes;
                        }
                                                            
            // 当前表
                                                    // 获取列表数据
            http.request('luxianCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function luxianTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=luxian_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.luxianTypesList = res.data.list;
               }
           });
       }
