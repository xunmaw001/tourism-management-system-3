            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">套餐展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="taocanaName" id="taocanaName" placeholder="套餐名" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="taocanaTypes" id="taocanaTypes" lay-filter="taocanaTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in taocanaTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <ul>
                        <li v-for="(item,index) in dataList" v-bind:key="index">
                            <div class="ng-prslist-doc">
                                <div class="ng-prslist-img" @click="jump('../taocana/detail.jsp?id='+item.id)">
                                    <a href="#" class="ng-prsliik">
                                        <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.taocanaPhoto?item.taocanaPhoto.split(',')[0]:''" />
                                        </span>
                                ...
                                <div class="ng-prslist-tit" @click="jump('../taocana/detail.jsp?id='+item.id)"><span>{{item.taocanaName}}</span></div>
                                <div class="ng-prslist-tit" v-if="item.taocanaNewMoney"><span style="color: red;">{{item.taocanaNewMoney}} RMB</span></div>
                            </div>
                        </li>
                    </ul>
                </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 taocanaContent 字段去除无用字符 如果字段长度大于60  就截取到60
            taocanaContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        form.verify({
            ...
            ,taocanaMoneyEnd: function(value, item){ //value：表单的值、item：表单的DOM对象
                var taocanaMoneyStart = $("#taocanaMoneyStart").val();
                if(taocanaMoneyStart != null && taocanaMoneyStart != "" && taocanaMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(taocanaMoneyStart);
                    if(number<0){
                        return '套餐价格的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#taocanaName').val() != null && jquery('#taocanaName').val() != "" && jquery('#taocanaName').val() != "null" ) {
                param['taocanaName'] = jquery('#taocanaName').val();
            }
         
            var taocanaTypes = $("#taocanaTypes").find("option:selected").val();
            if (taocanaTypes != null && taocanaTypes != "" && taocanaTypes != "null") {
                param['taocanaTypes'] = taocanaTypes;
            }
         
            if (jquery('#taocanaMoneyStart').val() != null && jquery('#taocanaMoneyStart').val() != "" && jquery('#taocanaMoneyStart').val() != "null" ) {
                param['taocanaMoneyStart'] = jquery('#taocanaMoneyStart').val();
            }
            if (jquery('#taocanaMoneyEnd').val() != null && jquery('#taocanaMoneyEnd').val() != "" && jquery('#taocanaMoneyEnd').val() != "null" ) {
                param['taocanaMoneyEnd'] = jquery('#taocanaMoneyEnd').val();
            }
                                            // 获取列表数据
            http.request('taocana/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function taocanaTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=taocana_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.taocanaTypesList = res.data.list;
               }
           });
       }
